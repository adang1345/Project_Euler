...
It can be seen that there are 3 fractions between 1/3 and 1/2. How many fractions lie between 1/3 and 1/2 in the sorted
...
for denominator in range(2, 12001):
    for numerator in range(denominator // 3, denominator // 2 + 1):
        if Fraction(1, 3) < Fraction(numerator, denominator) < Fraction(1, 2):
            fracs.add(Fraction(numerator, denominator))
print(len(fracs))
