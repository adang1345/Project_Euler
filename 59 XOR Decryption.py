...
def ascii_to_text(l):
    """returns text represented by list of ascii codes l"""
    text_list = []
    for x in l:
...
# open and read encrypted message
a = open("59 Cipher.txt")
encrypted_message = [int(x) for x in a.read().split(",")]
a.close()

# generate list of possible keys
keys = []
...
for key in keys:
    decrypted_message = ascii_to_text(ascii_decrypt(encrypted_message, key))
    if "the" in decrypted_message and "be" in decrypted_message and "and" in decrypted_message:
